...
function GetServerNumber(serverIP, serverPort) {
    switch (`${serverIP}:${serverPort}`) {
        ...
        case "server.tycoon.community:30125":
            return "5"
        case "ca.tycoon.community:30120":
            return "6"
        case "ca.tycoon.community:30122":
            return "7"
        case "ca.tycoon.community:30123":
            return "8"
        case "ca.tycoon.community:30124":
            return "9"
        case "ca.tycoon.community:30125":
            return "A"
        default:
            return "idk"
    }
...
bot.on("ready", async () => {
    ...
    const HeistChannel = bot.channels.get("539643307001905162") //Gets heist channel

    function checkServer(index, donttrack) { //Find people heisting in server
        ...
        request(`http://${botconfig.ActiveServers[index][0]}:${botconfig.ActiveServers[index][1]}/status/map/positions.json`, function (error, response, body) { //url to get all players
            ...
            if (CurrentServerPoints >= 2) {
                ...
                getPlayerLocation(botconfig.ActiveServers[index][0], botconfig.ActiveServers[index][1], ToTrack, "None", 1)
    ...
    function getPlayerLocation(ServerIP, ServerPORT, Tracking, LastPosition, TimesAtLocation) {
        console.log("Getting player location", ServerIP, ServerPORT, Tracking, LastPosition, TimesAtLocation)
        request(`http://${ServerIP}:${ServerPORT}/status/map/positions.json`, function (error, response, body) { //Get players on server
            if (!error) { //No error
                ...
                body.players.forEach(player => { //Loop through all players
                    if (player[2] == Tracking[2]) { //if in game ID is equal to the one told to search
                        console.log("Found player")
                        if (player[5].group != "pigs_job") return HeistChannel.send("Player is no longer heisting");
                        ...
                        botconfig.heistLocations.forEach(coords => { //Go through all positions
                            if (coords.DistanceNeeded > distanceBetweenCoords(currentCoords.x, currentCoords.y, coords.x, coords.y)) { //if closer to coords than distance needed
                                ...
                                if (coords.name != LastPosition) { //If the name is not the same as the last position
                                    ...
                                    HeistChannel.send(`**${player[0]}** is currently at **${coords.name}** on **server ${GetServerNumber(ServerIP, ServerPORT)}**`) //say where they are
                                    ...
                                    if (alreadyThere.fields[0]) { //If theres more than just the person
                                        HeistChannel.send(alreadyThere).then(msg => { //Send embed
                                            checkForNewPlayers(msg, coords, playersThere, ServerIP, ServerPORT) //Check for more arriving players
                                        })
                                    }
                        ...
                        if (!closeToAnything && LastPosition != "transit") { //If the player isn't close to a heighst location and the last position isn't transit
                            ...
                            setTimeout(() => {
                                getPlayerLocation(ServerIP, ServerPORT, Tracking, "transit", 1); //after 6500 ms get the heist leaders position
                            }, 6500);
                        } else {
                            if (LastPosition == "transit") TimesAtLocation++;
                            console.log("Still not close to anything")
                            setTimeout(() => {
                                getPlayerLocation(ServerIP, ServerPORT, Tracking, LastPosition, TimesAtLocation); //after 6500 ms get the heist leaders position
                            }, 6500);
                        }
                    }
                });
    ...
    function checkForNewPlayers(msg, location, playersThere, ServerIP, ServerPORT) {
        console.log("Checking for new players")
        request(`http://${ServerIP}:${ServerPORT}/status/map/positions.json`, function (error, response, body) { //Get players
            if (error) return;
            ...
            if (alreadyThere.fields[0]) { //if at least one person
                msg.edit(alreadyThere).then(msg => { //Edit the old embed with new one
                    if (NewPeople) { //if people are still coming
                        setTimeout(() => {
                            checkForNewPlayers(msg, location, playersThere, ServerIP, ServerPORT) //after 2000ms check for more new players
                        }, 2000);
                    }
